...
# Library: call Anthropic Claude on AWS Bedrock to extract scene JSON + waypoints.
# Public API:
...
• For WAYPOINTS: ≥10 points, spacing ≤1.0, each ∈ S, no waypoint inside an obstacle, and no segment between adjacent waypoints intersects any obstacle edge.
...
GOAL
Return ONLY the final JSON (at the end) with agent, goal, obstacle vertices, AND waypoints (a polyline path).

DISCRETE COORDINATE SET
All coordinates MUST be chosen from the set S = [0.0, 0.5, 1.0, ..., 9.5, 10.0]. Never output values outside [0,10].
...
2) Adjacent waypoint spacing ≤ 1.0 in Euclidean distance.
3) All waypoint coordinates MUST lie in S and within [0,10].
4) No waypoint may lie inside any obstacle.
5) No line segment between adjacent waypoints may intersect any obstacle boundary.
...
def _point_in_poly(pt, poly):
    # Ray casting; works for rectilinear polygons too.
    x, y = pt
    inside = False
    n = len(poly)
    for i in range(n):
        x1,y1 = poly[i]
        x2,y2 = poly[(i+1)%n]
        # check if point on horizontal edge (treat as inside -> reject)
...
def _seg_intersect(a,b,c,d):
    # Proper segment intersection test (including colinear-on-segment)
    def orient(p,q,r):
