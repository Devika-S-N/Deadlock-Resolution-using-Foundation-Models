...
class NavigationController:
    def __init__(self):
        ...
        self.detour_fail_count = 0
        self.detour_mode = "NORMAL"  # or "ESCAPE"

        self.recent_dirs = []
        self.max_dir_memory = 8
        ...
        try:
            while np.linalg.norm(x - self.goal) > self.threshold and step_guard < max_steps:
                ...
                if obstacle_detected:
                    ...
                    # continue
                    prev_dist = np.linalg.norm(x - self.goal)
                    ...
                    new_dist = np.linalg.norm(x - self.goal)
                    self._update_detour_mode(prev_dist, new_dist)
    ...
    def _llm_only_detour(self, x, min_attempts=1):
        ...
        # retry_note = None
        retry_note = None
        if self.detour_mode == "ESCAPE":
            retry_note = (
                "ESCAPE MODE: ignore the goal temporarily. "
                "Move away from previous waypoints and obstacles. "
                "Exploration is more important than goal progress."
            )
        ...
        while attempt < max_attempts:
            ...
            if self._reject_clustered_waypoints(detour_points):
                retry_note = "Avoid the previously explored region. That area is a deadlock."
                continue
    ...
    def _update_detour_mode(self, prev_dist, new_dist):
        if new_dist >= prev_dist - 0.05:
            self.detour_fail_count += 1
        else:
            self.detour_fail_count = 0

        if self.detour_fail_count >= 3:
            self.detour_mode = "ESCAPE"
        else:
            self.detour_mode = "NORMAL"
    ...
    def _validate_against_original(self, x, raw_wps):
        ...
        for j, wp in enumerate(raw_wps, start=1):
            ...
            if self._direction_reused(wp, prev):
                self.logger.log(f"Reject WP{j}: direction already explored")
                continue
            ...
            self.recent_dirs.append(ang)
            self.recent_dirs = self.recent_dirs[-self.max_dir_memory:]
    ...
    def _reject_clustered_waypoints(self, new_wps, eps=0.5, centroid_eps=0.6):
        if not self.visited_waypoints:
            return False

        old = np.array(self.visited_waypoints)
        new = np.array(new_wps)

        # point-wise check
        for p in new:
            if np.min(np.linalg.norm(old - p, axis=1)) < eps:
                return True

        # centroid check
        old_centroid = np.mean(old, axis=0)
        new_centroid = np.mean(new, axis=0)
        if np.linalg.norm(old_centroid - new_centroid) < centroid_eps:
            return True

        return False

    def _direction_reused(self, wp, prev, angle_thresh=np.deg2rad(25)):
        v = wp - prev
        ang = np.arctan2(v[1], v[0])

        for a in self.recent_dirs:
            if abs(np.arctan2(np.sin(a - ang), np.cos(a - ang))) < angle_thresh:
                return True
        return False
