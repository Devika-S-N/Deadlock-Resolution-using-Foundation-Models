...
from astar_planner import query_detour_waypoints
...
class NavigationController:
    def __init__(self, different=False):
        ...
        # Pass real obstacles to A* planner
        from astar_planner import set_real_obstacles
        set_real_obstacles(self.obstacles_geom)
        self.logger.log(f"Passed {len(self.obstacles_geom)} real obstacles to A*")
    ...
    def _llm_only_detour(self, x, min_attempts=5):
        """
        SIMPLIFIED A* DETOUR: Only obstacle validation.
        Removed: yellow validation, progress validation, retry loops.
        """
        ...
        self.logger.log(f"LIDAR detected {len(red_hits)} obstacle hits")

        # Get accumulated LIDAR for visualization
        ...
        image_path = self.mem.save_clean_snapshot(
            agent=x,
            goal=self.goal,
            lidar_hits=all_accumulated_hits
        )

        # Call A* planner (single attempt - no retries)
        self.logger.log("Calling A* planner")
        ...
        detour_points = query_detour_waypoints(
            ...
            yellow_waypoints=None,
            logger=self.logger
        )

        # Check if A* found waypoints
        if not detour_points:
            self.logger.log("A* found no valid path")
            return None

        self.logger.log(f"A* generated {len(detour_points)} waypoints")

        # ONLY obstacle validation
        valid_chain = self._validate_against_original(x, detour_points)

        if valid_chain:
            self.logger.log(f"SUCCESS: {len(valid_chain)} waypoints validated")
            
            # Add to memory for visualization
            self.mem.add_current_detours(valid_chain)
            
            self.logger.log("===== DETOUR END =====")
            return valid_chain
        else:
            self.logger.log("FAILED: Waypoints intersect obstacles")
            return None
    ...
    def _generate_escape_waypoints(self, agent_pos, goal_pos, lidar_hits):
        """
        SOLUTION: Generate waypoints using geometric heuristic when astar_planner repeatedly fails.
        
        When astar_planner fails 5 times in a row, it's likely stuck trying the same strategy.
        ...
        """
        self.logger.log("ESCAPE HEURISTIC: astar_planner failed - using geometric waypoint generation")
