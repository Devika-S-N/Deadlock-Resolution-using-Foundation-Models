...
def generate_env(
    ...
    while True:
        for _ in range(max_global_restarts):
            ...
            img, meta = res
            out_png = os.path.join(out_dir, f"env_{stamp()}_N{N_current}_R{num_rooms}_O{num_obstacles}.png")
        ...
        if not auto_grow or N_current >= max_N:
            raise RuntimeError("Could not generate a valid environment within auto-grow limit.")
